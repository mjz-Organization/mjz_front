...
<script>
  export default {
    ...
    mounted(){
        let left = this;
        this.get(ApiPath.system.selectAd,{"page":left.currentPage,"per_page":left.pageCount,"record_type":1})
        .then(function(res){
            if(res.data.code == 0){
    ...
    methods: {
        ...
        handleEdit(index, row) {
            ...
            this.$router.push({path: ApiPath.system.businessStartPageAdd,query:{id:id,images_id:images_id,ad_name:ad_name,path:path,types:types,content:content}});
        },
        ...
        //跳转到新增启动页页面
        addStartPage(){
            this.$router.push({path: ApiPath.system.businessStartPageAdd});
        },
